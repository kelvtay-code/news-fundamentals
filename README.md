...
- **Research Team** — latest three-agent report (`research_team_*.html` in Dashboard) chaining fundalist (business overview) → technicalist (Dual_X, GEX, IV, 5d/30d volume expansion) → sentilist (news bulletin rating, plus a 7+ catch-all for tickers the first two agents missed), synthesized into one report with proposed trades and GEX/put-wall/call-wall commentary. See `.claude/skills/research-team/SKILL.md` for the pipeline spec.
